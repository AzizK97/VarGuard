...
if [ "$APPLY" -eq 1 ]; then
  ...
  kubectl create configmap suricata-config -n "$NAMESPACE" \
    --from-file=suricata.yaml="$RENDERED" --dry-run=client -o yaml | kubectl apply -f -
  # ensure rules configmap updated from repo rules
  if [ -f docker/suricata/custom.rules ]; then
    kubectl create configmap suricata-rules -n "$NAMESPACE" \
      --from-file=custom.rules=docker/suricata/custom.rules --dry-run=client -o yaml | kubectl apply -f -
  fi
  kubectl rollout restart deployment/suricata -n "$NAMESPACE"
  echo "Applied and restarted. Check: kubectl get pods -n $NAMESPACE"
else
  echo "Local-only mode (no cluster apply). To apply run:"
  echo "  kubectl create configmap suricata-config -n $NAMESPACE --from-file=suricata.yaml=$RENDERED --dry-run=client -o yaml | kubectl apply -f -"
  echo "  kubectl apply -f $SURICATA_DEPLOY_YAML && kubectl rollout restart deployment/suricata -n $NAMESPACE"
fi
